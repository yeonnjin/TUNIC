...
HRESULT CObject_Gem::Render_LightDepth()
{
...
void CObject_Gem::Move_Pop(_float fTimeDelta)
{
    ...
    if (vPosition.m128_f32[1] <= fHeight)
    {
        ...
        m_isLand = true;
        m_pGameInstance->Play_Once(TEXT("OBJECT_Gem_Land.wav"), CSound_Manager::EFFECT1);
    }
...
void CObject_Gem::Move_Collision(_float fTimeDelta)
{
    ...
    if (true == m_pTransformCom->Move_To_Target(vPlayerPosition, fTimeDelta * 3.f, 0.5f))
    {
        m_isDead = true;
        pPlayer->Set_Gem();
    }
...
void CObject_Gem::Collision_Event(Engine::CGameObject* pGameObject)
{
    ...
    {
        m_isCollision = true;
        m_pGameInstance->Play_Loop(TEXT("OBJECT_Gem_Get.wav"), CSound_Manager::EFFECT1);
    }
}
