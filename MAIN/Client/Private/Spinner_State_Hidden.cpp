...
void CSpinner_State_Hidden::OnStateUpdate(_float fTimeDelta)
{
    ...
    if (false == m_isEmerge && 17 > XMVector3Length(vPlayerPosition - vMonsterPosition).m128_f32[0])
    {
        m_pMonster->Set_Blending(true, CMonster_Spinner::ANIM_EMERGE);
        m_isEmerge = true;
        m_pMonster->Set_isAggro(true);
        m_pGameInstance->Play_Once(TEXT("MONSTER_Spinner_Aggro.wav"), CSound_Manager::MONSTER2);
    }
